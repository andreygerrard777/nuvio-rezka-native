# Версія 3: перевірка Anubis

Один нативний провайдер виконує GET → SHA-256 proof-of-work → pass-challenge → повторний GET зі збереженими cookie. Маркер у тесті Nuvio: `REZKA_V3`.

`result=OK` означає HTTP 2xx, відсутність Anubis та наявність розмітки каталогу/плеєра. Це ще не перевірка потоків. `CHALLENGED`, `PASS_REJECTED`, `UNKNOWN_PAGE` або `stage=... error=...` показують причину зупинки. Cookie, токени й HTML не включаються в повідомлення.

Одна спроба challenge; складність 0–4, обчислення до 5 секунд, загальний бюджет мережевої перевірки 40 секунд. Переходи лише в межах того самого origin. Тестування: `:RezkaDiagnostics:testDebugUnitTest`, включно з 8 новими локальними сценаріями Anubis/302. Kotlin-тести цієї версії потрібно виконати в GitHub Actions; на телевізорі версія 3 ще не перевірена.

У Nuvio після оновлення має бути Version 3. Пакет усе ще діагностичний: `0 streams` очікується. Попередні інструкції нижче описують початкові етапи; маркери v1/v2 замінені на `REZKA_V3`, а провайдер тепер один.
